...
public class MelonCaveGen extends MapGenBase
{
    /**
    ...
    public void generate(World par2World, int par3, int par4, byte[] par5ArrayOfByte)
    {
        int k = this.range;
                    ...
                    for (j3 = l1; !flag2 && j3 < i2; ++j3)
                    {
                        for (int l3 = l2; !flag2 && l3 < i3; ++l3)
                        {
                            for (int i4 = k2 + 1; !flag2 && i4 >= j2 - 1; --i4)
                            {
                                ...
                                if (i4 >= 0 && i4 < 128)
                                {
                                    if (par5ArrayOfByte[k3] == MelonCraft.melonjuice.blockID || par5ArrayOfByte[k3] == Block.waterStill.blockID)
                                    {
                                        flag2 = true;
                                    }
                                ...
                                if (d12 * d12 + d13 * d13 < 1.0D)
                                {
                                    for (int k4 = k2 - 1; k4 >= j2; --k4)
                                    {
                                        ...
                                        if (d14 > -0.7D && d12 * d12 + d14 * d14 + d13 * d13 < 1.0D)
                                        {
                                            ...
                                            if (b0 == MelonCraft.melonGrass.blockID)
                                            {
                                                flag3 = true;
                                            }

                                            if (b0 == MelonCraft.melonStone.blockID || b0 == MelonCraft.melonDirt.blockID || b0 == MelonCraft.melonGrass.blockID)
                                            {
                                                if (k4 < 10)
                                                {
                                                    par5ArrayOfByte[j4] = (byte)MelonCraft.magilava.blockID;
                                                }
                                                else
                                                {
                                                    par5ArrayOfByte[j4] = 0;

                                                    if (flag3 && par5ArrayOfByte[j4 - 1] == MelonCraft.melonDirt.blockID)
                                                    {
    ...
    protected void recursiveGenerate(World par1World, int par2, int par3, int par4, int par5, byte[] par6ArrayOfByte)
    {
        ...
        //System.out.println("GENERATE MELONCAVE: RECURSIVE GENERATE");
        ...
        {
        	//System.out.println("GENERATE MELONCAVE: CAVE WILL FORM");
            ...
            {
            	//System.out.println("GENERATE MELONCAVE: LARGE CAVE NODE");
            	System.out.println("LARGE par4: "+par4+" par5: "+par5);
            ...
            for (int l1 = 0; l1 < k1; ++l1)
            {
                ...
                //System.out.println("GENERATE MELONCAVE: ABOUT TO GENERATE CAVE NODE");
                System.out.println("par4: "+par4+" par5: "+par5);
